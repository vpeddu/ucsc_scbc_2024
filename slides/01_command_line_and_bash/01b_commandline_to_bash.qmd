...
What are some things we could do with/to `FASTA` files using the `commandline`?
...
Commandline has yet another built-in tool that can help us out

-   Your `final_gene_panel.csv` should look something like:

## From command line to Bash

This is a bit of a misnomer, as we're already using the `Bash commandline`.

However, we're going to start moving away from doing *everything* on the commandline

Why do you think?

## Saving the code you run and re-using it
